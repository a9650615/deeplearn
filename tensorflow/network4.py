...
# images_train = data['raw']
# labels_train = data['labels']

def trans_label_to_label_array(label_train):
    new_arr = np.array([])
    for index in range(len(labels_train)):
        tmp_arr = np.zeros(10)
#         tmp_arr[labels_train[index]] == 1
        np.put(tmp_arr, labels_train[index], 1, mode='clip')
...
# labels_train = trans_label_to_label_array(labels_train)
#读取测试数据集
...
#用于输入图像的占位变量
x = tf.placeholder(tf.float32, shape=[None, num_channels, img_size, img_size], name='x')
...
def pre_process_image(image, training):
    ...
    if training:
        ...
        image = tf.random_crop(image, size=[num_channels, img_size_cropped, img_size_cropped])
